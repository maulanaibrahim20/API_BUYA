...
// Metode GET untuk autentikasi driver
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    if (isset($_GET['telepon']) && isset($_GET['password'])) {
        ...
        // Query untuk mengecek data driver berdasarkan telepon dan password
        $sql = "SELECT id_driver, nama, telepon, email FROM driver WHERE telepon = '$telepon' AND password = '$password'";
        $result = $conn->query($sql);

        // Jika driver ditemukan
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $json['data'][] = $row; // Simpan data driver dalam array
        } else {
            // Jika data driver tidak ditemukan
